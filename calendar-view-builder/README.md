...
| `eventSortOrder` | `Category ↓, Status ↓, Alphabetical ↓` | *(blank)* | Sort order for events within a day. Dropdown of `Category / Status / Type / Alphabetical / Source` × `↓ / ↑`. Enable Allow-multiple-selections on the cell to stack sorts (primary, secondary, ...). `↓` = ascending, `↑` = descending. Blank Key cell falls back to the script default. |
